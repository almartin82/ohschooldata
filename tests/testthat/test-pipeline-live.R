# ==============================================================================
# LIVE Pipeline Tests for ohschooldata
# ==============================================================================
#
...
test_that("Ohio DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Ohio DOE URL and verify HTTP 200")
})
...
test_that("Can download Ohio enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Ohio enrollment file", {
  skip_if_offline()
...
test_that("ohschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- ohschooldata::get_available_years()
  ...
  tryCatch({
    raw <- ohschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- ohschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- ohschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- ohschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- ohschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- ohschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- ohschooldata:::get_cache_path(2024, "enrollment")
